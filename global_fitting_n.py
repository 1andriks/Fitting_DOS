...
def read_txt(t,dt,Nmod,Nmodtext,ff,r,DOS_space,grid,version,wvlen, i):
    global delta_w
    dirName='/Users/erglisa/Desktop/SHAHEEN_n/WVLEN=' + str(wvlen)+'/MOD_n=' + \
    ...
    results_half = peak_widths(norm_data, peaks, rel_height=0.3) #width of peaks
...
# i=3.4
# read_txt(150001, 5.89664e-12 ,50*50 ,'50x50', i, 0.04, '0.4x1.6_0.4_1.6', '400x400', i, 0.5, 1) #78

delta_w_list = []
delta_w_mean_list = []
# i=0.15
files = [i+1 for i in range(90)]
n_index = [1.3, 1.6, 1.9,2.2, 2.5, 2.8, 3.1, 3.4, 3.7, 4]
# n_index = [1.3, 1.6, 1.9]
...
for i in n_index:
    ...
    for version in range(90):
        try:
            read_txt(150001, 5.89664e-12 ,50*50 ,'50x50', i, 0.04, '0.4x1.6_0.4_1.6', '400x400', i, 0.5, version+1) 
        except:
            print('fail at version=', version, 'and n_index =', i)
            continue
...
plt.errorbar(n_index, delta_w_mean_list, yerr=delta_w_stdev, label='Q_avg', fmt='o', color='tab:red')
plt.xlabel('Refractive index')
plt.ylabel('delta w')
plt.suptitle('peak width with increasing n at ff=0.27')
plt.show()
